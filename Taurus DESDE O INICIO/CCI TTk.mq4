...
input int                CCI_Period           = 6;                    // Cci period
input ENUM_APPLIED_PRICE CCI_Price            = PRICE_OPEN;         // Cci price
input double             levelOb              = 160;                   // Level overbought
input double             levelOs              = -160;                  // Level oversold
                ...
                const int &spread[])
{
   ...
   for (; i>=0 && !_StopFlag; i--)
   {
      ...
      {
            if (trend[i] ==  1) upArr[i] = low[i] - iATR(_Symbol,_Period,15,i)*inpArrowGapUp;
