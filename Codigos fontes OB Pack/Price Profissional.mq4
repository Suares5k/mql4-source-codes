...
extern int Dias = 5; // Dias do backtest
...
extern simnao SeR = NAO;     // Ativar Leitura Dom forex?
...
extern string                CCISettings              = "--------------------";//[___ CCI Settings ___]
extern int                   CCI_Period_2               = 6;                     // CCI Period
extern ENUM_APPLIED_PRICE    Apply_to_2                 = PRICE_TYPICAL;         // CCI Applied Price
extern int                   CCI_Overbought_Level     = 180;                   // CCI Overbought Level
extern int                   CCI_Oversold_Level       = -180;                  // CCI Oversold Level
// .
extern string                MASettings               = "----------";          // [.:: Moving Average Settings ::.]
extern bool                  UseSMAFilter             = false;                  // Enable Using SMA Filter
extern int                   MA_Period                = 20;                   // MA Period
extern int                   MA_Shift                 = 0;                     // MA Shift
extern ENUM_MA_METHOD        MA_Method                = MODE_SMMA;              // MA Method
extern ENUM_APPLIED_PRICE    MA_Applied_Price         = PRICE_CLOSE;           // MA Applied Price
extern int                   FilterShift              = 1;                     // MA Filter Shift
...
//PAINEL
   if(AtivaPainel && D1!=iTime(Symbol(),Period(),0)) {
      ...
      //  Print("ESTATÍSTICAS");
      string estatisticas;
      CommentLab("ANUBIS PROFISSIONAL V4", 35, 20, 1,clrWhite);
      CommentLab(Symbol()+": ESTATÍSTICAS", 65, 20, 2,clrLime);
      estats.Sort();
      ...
      CommentLab("DOJI : " +doji, 80, 20, 3,clrWhite);
      ...
      for(int a=1; a<MartingaleBacktest+1; a++) {
         ...
         if(total_win_gale < 50) {
            CommentLab("GALE " +a+ " "+vitorias+" "+" X "+perdas+ " - "+total_win_gale+"%", 115+15*a, 20, a+100,clrLime);
...
void OnTimer() {
   ...
   ObjectSetText("Time_Remaining", ""+mText+":"+sText, 13, "Verdana", StrToInteger(mText+sText) >= 0010 ? clrYellow : clrGold);
