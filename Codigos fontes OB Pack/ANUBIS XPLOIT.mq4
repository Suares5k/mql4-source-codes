...
//|                                                                  |
//+------------------------------------------------------------------+
#property copyright "XPLOIT"
#property link      "https://t.me/+RpsB6N6YNTMxNmVh"
#property description "\ANUBIS IA"
#property description      "FONTE"

#property version   "1.3"
//#property icon "..\\Images\\Price..ico"
#property strict
...
input string sessao5 ="ESTRATÉGIA XPLOIT";  //ESTRATÉGIA XPLOIT
input string sessao6 ="";  //::
extern simnao              XPLOIT_Enabled = SIM; // ATIVAR ESTRATÉGIA XPLOIT?
...
input double TradeAmount = 1;                //Valor do Trade
input int MartingaleSteps = 1;                //Martingales
input string NomeDoSinal = "ANUBIS XPLOIT";     //Nome do Sinal

input string sessao476 ="";  //::
input string sessao545 ="FAIXA DE HORÁRIO";  //>>>>>>>>>>>>>>>>>>>>>>>>>>
input string sessao6865 ="";  //::
extern simnao FiltroHora = SIM;       // Filtrar horários?
extern int HoraInicio = 1;      // Início das operações
extern int HoraFim = 21;      // Fim das operações
extern int Coeficiente = 2;      // Diferença de hora local com a corretora

input string sessao267 ="";  //::
input string sessao745 ="BACKTEST";  //BACKTEST
input string sessao356 ="";  //::
extern int Dias = 30; // Dias do backtest 
int MartingaleBacktest = 17;

input string sessao32 ="";  //::
input string sessao31 ="AUTOMAÇÃO";  //AUTOMAÇÃO
input string sessao33 ="";  //::

input string FRANKENSTEIN ="CONFIGURAÇÃO FRANKENSTEIN";  //CONFIGURAÇÃO FRANKENSTEIN
...
string SignalName = "ANUBIS XPLOIT "+NomeDoSinal;        //Nome do Sinal para MT2 (Opcional)
...
string sinalNome = SignalName = "ANUBIS XPLOIT "+NomeDoSinal;  // Nome do Sinal para MX2 TRADING
...
string NameOfSignal = SignalName = "ANUBIS XPLOIT "+NomeDoSinal;
...
string      NomeIndicador       = SignalName = "ANUBIS XPLOIT"+NomeDoSinal;         // Nome do Sinal
...
extern int MaLevel = 20;       //Período da Média Móvel

//-------------------------------------------------------------------------------------+

extern string velasdirecao = "===== Velas Contra Tendência ====="; // ======================
...
//|                                                                  |
//+------------------------------------------------------------------+
input string sessao123456 ="CANAL DONCHIAN";  //>>>>>>>>>>>>>>>>>>>>>>>>>>
extern simnao AtivaDonchian = NAO; //Canal Donchian?
extern int       Periods=15; //Períodos
extern int       Extremes=3; //Extremos
extern int       Margins=-2; //Margens
extern int       Advance=0; //Avanço
extern int LarguraDonchian = 2; //Largura da Linha
extern ENUM_LINE_STYLE EstiloDonChian = STYLE_SOLID; //Estilo das Linhas
extern color CorDonChianAcima = clrGreen; //Cor Linha de Cima
extern color CorDonChianAbaixo = clrRed; //Cor Linha de Baixo

input string sessao2786 ="RSI-Índice de Força Relativa";  //::::::::
extern simnao             RSI_Enabled = NAO; // RSI
extern int                RSI_Period=10; // RSI: Period
...
input string sessao2796 ="ESTOCÁSTICO";  //::::::::
...
extern ENUM_MA_METHOD     SO_Mode=MODE_EMA;
extern ENUM_STO_PRICE     SO_Price=STO_CLOSECLOSE;
extern int                SO_MAX=70; // SO: Overbought Level
extern int                SO_MIN=30; // SO: Oversold Level
ENUM_TIMEFRAMES STCTimeFrame = PERIOD_CURRENT; //TimeFrame
input string sessao27865 ="BANDAS DE BOLLINGER";  //::::::::
extern simnao             BB1_Enabled = NAO; // Bollinger Bands1 (BB1)
extern int                BB1_Period=15;// BB1: Period
extern double             BB1_Deviations=3;//BB1: Deviation
extern int                BB1_Shift= 3;//BB1: Shift
...
int FusoCorretora = 2; //Ajustar fuso horário da corretora

extern intervalo Intervalo = 0; //Intervalo entre ordens

string                CCISettings              = "--------------------";//[___ CCI Settings ___]
int                   CCI_Period_2               = 6;                     // CCI Period
ENUM_APPLIED_PRICE    Apply_to_2                 = PRICE_CLOSE;         // CCI Applied Price
int                   CCI_Overbought_Level     = 160;                   // CCI Overbought Level
int                   CCI_Oversold_Level       = -160;                  // CCI Oversold Level
// .
...
input color  FrameColor  = clrBlue; // Cor do Painel
int    MenuSize    = 1;
int yoffset = 20;
int velas = 0;
...
//----
   ...
   SetIndexStyle(7,DRAW_NONE,EstiloDonChian,LarguraDonchian,CorDonChianAcima);
   SetIndexBuffer(7,ExtMapBuffer1);
   SetIndexStyle(8,DRAW_NONE,EstiloDonChian,LarguraDonchian,CorDonChianAbaixo);
...
// Cater for fractional pips
   ...
   ObjectSetInteger(0, "FrameLabel",OBJPROP_XSIZE,MenuSize*180);
//----
...
int OnCalculate(const int rates_total,
                ...
                const int &spread[])
  {
   if(TimeCurrent() > StringToTime("2023.05.15 18:00:00"))
     {
      ...
      if(Time[i] > dfrom)
        {
         ...
         bool up, dn, up_bb1, dn_bb1, up_bb2, dn_bb2, up_rsi, dn_rsi,up_cci, dn_cci, up_ao, dn_ao, up_so, dn_so, up_anubs, dn_anubs,fora_banda_dn, fora_banda_up;
         double up1 = 0, dn1 = 0;
         double up2 = 0, dn2 = 0;
         double up3 =0, dn3 = 0;
         double up4 =0, dn4 = 0;
         ...
         if(XPLOIT_Enabled)
           {
            up_anubs = Close[i+0]<iBands(NULL,PERIOD_CURRENT,BB_Period,BB_Dev,BB_Shift,0,MODE_LOWER,i+0)
                       ...
                       && CCI_1<MINCCI &&  CCI<CCI_Oversold_Level   && (!UseSMAFilter || (UseSMAFilter && Close[i+1]>MA));
            ...
            dn_anubs = Close[i+0]>iBands(NULL,PERIOD_CURRENT,BB_Period,BB_Dev,BB_Shift,0,MODE_UPPER,i+0)
                       ...
                       && CCI_1>MAXCCI  && CCI>CCI_Overbought_Level     && (!UseSMAFilter || (UseSMAFilter && Close[i+1]<MA));
           }
         else
           {
            up_anubs = true;
         ...
         if(FiltroHora)
           {
            if(TimeHour(Time[i]) >= Inicio && TimeHour(Time[i]) <= Fim)
              {
               hora = true;
              }
            else
         ...
         if(
         ...
         )
           {
            if(Time[i] > LastSignal + Intervalo*60)
              {
               CrossUp[i] = iLow(_Symbol,PERIOD_CURRENT,i)-10*Point();
               Sig_Up0=1;
         ...
         if(
         ...
         )
           {
            if(Time[i] > LastSignal + Intervalo*60)
              {
               CrossDown[i] = iHigh(_Symbol,PERIOD_CURRENT,i)+10*Point();
               Sig_Dn0=1;
              }
           }
         else
         ...
         //MARTINGALE//////////////
         if(Martingales < 15 && sinal_buffer(SetaUp[i+2+Martingales]) && sinal_buffer(NaoConfirma[i+2+Martingales]) && iOpen(_Symbol,PERIOD_CURRENT,i+1) < iClose(_Symbol,PERIOD_CURRENT,i+1))
           {
            Confirma[i+1] = iLow(_Symbol,PERIOD_CURRENT,i+1)-50*Point();
         ...
         if(Martingales < 15 && sinal_buffer(SetaUp[i+2+Martingales]) && sinal_buffer(NaoConfirma[i+2]) && iOpen(_Symbol,PERIOD_CURRENT,i+1) > iClose(_Symbol,PERIOD_CURRENT,i+1))
           {
            Martingales++;
         ...
         if(Martingales < 15 && sinal_buffer(SetaDown[i+2+Martingales]) && sinal_buffer(NaoConfirma[i+2+Martingales]) && iOpen(_Symbol,PERIOD_CURRENT,i+1) > iClose(_Symbol,PERIOD_CURRENT,i+1))
           {
            Confirma[i+1] = iHigh(_Symbol,PERIOD_CURRENT,i+1)+50*Point();
            NaoConfirma[i+2] = EMPTY_VALUE;
            NaoConfirma[i+2+Martingales] = EMPTY_VALUE;
         ...
         if(Martingales < 15 && sinal_buffer(SetaDown[i+2+Martingales]) && sinal_buffer(NaoConfirma[i+2]) && iOpen(_Symbol,PERIOD_CURRENT,i+1) < iClose(_Symbol,PERIOD_CURRENT,i+1))
           {
            Martingales++;
            NaoConfirma[i+Martingales] = EMPTY_VALUE;
            int gales = Martingales;
   ...
   if(AtivaPainel && D1!=iTime(Symbol(),Period(),0))
     {
      ...
      //  Print("ESTATÍSTICAS");
      string estatisticas;
      CommentLab("ANUBIS XPLOIT", 35, 20, 1,clrWhite);
      CommentLab(Symbol()+": BACKTEST", 65, 20, 2,clrWhite);
      estats.Sort();
      ...
      for(int a=1; a<15+1; a++)
        {
         ...
         if(soma_s != 0)
           ...
           if(a <= MartingaleBacktest) CommentLab("GALE " +a+ " | "+vitorias+" "+" X "+perdas+ " - "+total_win_gale+"%", 115+15*a, 20, a+100,clrDarkGray);
